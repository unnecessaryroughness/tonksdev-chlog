$(function(){    
    $("#cmdNew").on("click", function() { 
        ...
        $("#modalcontent #cmdAdd").on("click", function() { 
            if ($("#modalcontent #txtNewSideEffect").val().length > 0) {
                addRecord($("#modalcontent #txtNewSideEffect").val());    
            }
            modalwin.close();
        });
        $("#modalcontent #txtNewSideEffect").focus();
    });
...
function displayRecords(){
    $("#tblSideEffects").html("");
    $("#tblSideEffects").append('<tr class="header">' +
    ...
    for (i=0; i<jso.record.length; i++) {
        ...
        $("#tblSideEffects").append("<tr id='row_" + i + "' class='row'></tr>");
